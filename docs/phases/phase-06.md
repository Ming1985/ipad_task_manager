...
| Safari 等系统 App 无法屏蔽 | 系统限制，无法绕过 |
| 白名单 App 在任务期间也被屏蔽 | 白名单功能需 DeviceActivityMonitor 实现，延后到 Phase 07 |
| SwiftData Relationship 自动去重导致重复任务丢失 | 使用 taskOrder 存储完整顺序（含重复） |
...
### 手动验证 (真机必需)
- [x] FamilyControls 权限请求弹窗正确
- [x] 授权后屏蔽功能正常（黑名单模式生效，白名单延后）
- [ ] 拒绝后降级处理
- [x] 权限状态持久化
- [ ] 任务开始时非白名单 App 被屏蔽 - 延后到 Phase 07
- [x] 任务结束后屏蔽移除
- [ ] 永久屏蔽列表生效 - 待测试
